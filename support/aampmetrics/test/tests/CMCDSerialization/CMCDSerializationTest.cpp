 ...
 * serializer and cause every assertion to pass vacuously.
 *
 * Tests cover CTA-5004 §3 requirements:
 *   - br/tb rounded to nearest 100 kbps
 *   - bl rounded to nearest 100 ms
 *   - sid/nor/nrr serialized as quoted-string tokens
 *   - keys alphabetically sorted within each group
 *   - exact CMCD header group key names (with single trailing ':')
 *   - bs emitted as bare token when starving; omitted when not
 *   - each key appears in its correct CTA-5004 header group
 *   - Comcast custom keys (com.comcast-dns/fb/lb) retained in CMCD-Request
 *   - emission is purely a function of BuildCMCDCustomHeaders; the on/off
...
// ---------------------------------------------------------------------------
// br and tb rounded to nearest 100 kbps
// ---------------------------------------------------------------------------

/**
 * Video subclass rounds br and tb to nearest 100 kbps.
...
/**
 * tb rounds down: SetTopBitrate(5942) -> tb=5900.
 */
...
/**
 * bufferLength in ms is rounded to nearest 100.
...
/**
 * bl rounds down: SetBufferLength(2349) -> bl=2300.
 */
...
// ---------------------------------------------------------------------------
// quoted-string token types
// ---------------------------------------------------------------------------

/**
 * sid is wrapped in double-quotes (quoted-string, not a bare token).
 * v=1 is always present alongside sid.
 */
...
/**
 * nor value is wrapped in double-quotes in CMCD-Request.
 */
...
/**
 * nrr value is wrapped in double-quotes in CMCD-Request.
 */
...
/**
 * Combined: CMCD-Object value is exactly "br=3800,ot=v,tb=6000"
...
/**
 * CMCD-Request keys sorted: bl < com.comcast-dns < com.comcast-fb < com.comcast-lb < nor.
...
// ---------------------------------------------------------------------------
// exact CMCD group key names with single trailing ':'
// ---------------------------------------------------------------------------

/**
 * The four group map keys are exactly "CMCD-Object:", "CMCD-Request:",
...
// ---------------------------------------------------------------------------
// bs bare token present/absent based on bufferStarvation
// ---------------------------------------------------------------------------

/**
 * bufferStarvation=true emits CMCD-Status: with value "bs" (bare token, no '=').
 */
...
/**
 * bufferStarvation=false — "CMCD-Status:" key must be absent from the map.
 */
...
/**
 * Latched semantics: a starvation observed since the prior request must still be reported
 * even after a later non-starving sample. SetBufferStarvation(false) must not clear a latch.
 */
TEST(CMCDSerialization_Status, BsLatchedAcrossNonStarvingSample)
{
    VideoCMCDHeaders v;
    v.SetSessionId("test-sid");
    v.SetBufferStarvation(true);   // starvation seen between requests
    v.SetBufferStarvation(false);  // buffer recovered before the request is built

    auto headers = BuildHeaders(v);
    EXPECT_EQ(JoinedValue(headers, "CMCD-Status:"), "bs")
        << "a latched starvation must survive a later non-starving sample";
}

/**
 * Consume-on-emit: bs is reported once on the resumption request, then cleared. A subsequent
 * request with no new starvation must omit bs.
 */
TEST(CMCDSerialization_Status, BsClearedAfterReport)
{
    VideoCMCDHeaders v;
    v.SetSessionId("test-sid");
    v.SetBufferStarvation(true);

    auto first = BuildHeaders(v);
    EXPECT_EQ(JoinedValue(first, "CMCD-Status:"), "bs");

    auto second = BuildHeaders(v);
    EXPECT_EQ(second.find("CMCD-Status:"), second.end())
        << "bs must report once then clear until the next starvation";
}

// ---------------------------------------------------------------------------
// key→group mapping correctness
// ---------------------------------------------------------------------------

/**
 * ot/br/tb appear only in CMCD-Object; bl/nor/com.comcast-* only in CMCD-Request;
...
// ---------------------------------------------------------------------------
// Comcast custom keys retained in CMCD-Request
// ---------------------------------------------------------------------------

/**
 * com.comcast-dns, com.comcast-fb, com.comcast-lb all appear in
...
/**
 * nrr branch: com.comcast-fb and com.comcast-lb present when using
...
TEST(CMCDSerialization_Video, BaseSessionAndSubclassGroupsCoexist)
{
    ...
    // Base CMCD-Session: must be present and correctly quoted; v=1 always present
    EXPECT_EQ(JoinedValue(headers, "CMCD-Session:"), "sid=\"6e2fb550-c457\",v=1");
...
/**
 * ManifestCMCDHeaders emits ot=m in CMCD-Object and a quoted
 ...
 * directly; gate coverage is a collector-level concern.
 */
...
/**
 * SubtitleCMCDHeaders emits ot=s in CMCD-Object and a quoted sid.
 */
...
/**
 * AudioCMCDHeaders emits ot=a for normal audio and ot=i for init segments.
 */
...
/**
 * omit-zero: br and tb that round to zero are omitted from CMCD-Object.
...
/**
 * empty-state path: calling BuildCMCDCustomHeaders on a default-constructed
...
TEST(CMCDSerialization_EmptyState, DoesNotCrash)
{
    ...
    EXPECT_EQ(JoinedValue(headers, "CMCD-Object:"), "ot=m");
    // CMCD-Session: must be present (sid="" with v=1 always present)
    EXPECT_EQ(JoinedValue(headers, "CMCD-Session:"), "sid=\"\",v=1");
}

// ---------------------------------------------------------------------------
// Regression: nor must not appear when nextUrl is empty
// ---------------------------------------------------------------------------

/**
 * Default-constructed VideoCMCDHeaders (no nextUrl set) must not
...
/**
 * Regression (Audio): same guard applies to AudioCMCDHeaders.
 */
...
// ---------------------------------------------------------------------------
// v: version key always present in CMCD-Session
// ---------------------------------------------------------------------------

/** v=1 is always present in CMCD-Session alongside sid (no other keys set). */
...
// ---------------------------------------------------------------------------
// sf: streaming format present when set, absent when empty
// ---------------------------------------------------------------------------

/** SetStreamingFormat("h") -> sf=h in CMCD-Session (HLS). */
...
/** SetStreamingFormat("d") -> sf=d in CMCD-Session (DASH). */
...
/** SetStreamingFormat("s") -> sf=s in CMCD-Session (Smooth Streaming). */
...
/** No SetStreamingFormat call -> sf= must be absent from CMCD-Session. */
...
// ---------------------------------------------------------------------------
// st: stream type present when set, absent when empty
// ---------------------------------------------------------------------------

/** SetStreamType("l") -> st=l in CMCD-Session (live). */
...
/** SetStreamType("v") -> st=v in CMCD-Session (VOD). */
...
/** No SetStreamType call -> st= must be absent from CMCD-Session. */
...
// ---------------------------------------------------------------------------
// pr: playback rate present (no trailing zeros) at non-1x, absent at 1x
// ---------------------------------------------------------------------------

/**
 * SetPlaybackRate(2.0f) -> pr=2 (no trailing zero ".0").
 * Locks the snprintf "%g" trailing-zero strip.
 */
...
/** SetPlaybackRate(0.5f) -> pr=0.5 in CMCD-Session (fractional rate). */
...
/** Default playback rate (1.0f, no SetPlaybackRate call) -> pr= must be absent. */
...
// ---------------------------------------------------------------------------
// cid: content id quoted when set, absent when empty
// ---------------------------------------------------------------------------

/** SetContentId sets a quoted-string cid token in CMCD-Session. */
...
/** No SetContentId call -> cid= must be absent from CMCD-Session. */
...
// ---------------------------------------------------------------------------
// alpha-sort lock-in: cid<pr<sf<sid<st<v with all six keys
// ---------------------------------------------------------------------------

/**
 * All six CMCD-Session keys present (live HLS with cid,
...
/**
 * DASH VOD at 2x trick-play, no cid.
...
/**
 * With sf and st set, confirm sf < sid < st ordering in
...
// ---------------------------------------------------------------------------
// Regression: SerializeToCMCDMap merges rather than overwrites same group
// ---------------------------------------------------------------------------

/**
 * Two sequential SerializeToCMCDMap calls that both target CMCD-Session
...
/**
 * All six CMCD-Session keys present simultaneously (live HLS,
...
// ---------------------------------------------------------------------------
// d — object duration, NOT rounded, media-only
// ---------------------------------------------------------------------------
 ...
 * and NOT "d=2200". Locks the "d is plain ms, never rounded" invariant.
 */
...
// ---------------------------------------------------------------------------
// dl — deadline, rounded 100 ms, scaled by rate
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// su — startup-urgent bare bool token
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Full-set lock-in — all new keys in a representative Video request
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// br/tb/mtp/rtp rounded to nearest 100 kbps (new-key angles)
// ---------------------------------------------------------------------------

/**
 * mtp rounded to nearest 100 kbps.
 * SetMeasuredThroughput(4842) -> mtp = (4842+50)/100*100 = 4800.
 * Named test to explicitly document the rounding contract for mtp.
 */
...
/**
 * rtp rounded to nearest 100 kbps.
 * SetBitrate(3842) -> rtp = 3842*2 = 7684 -> (7684+50)/100*100 = 7700.
 * Named test to explicitly document the rounding contract for rtp.
 */
...
// ---------------------------------------------------------------------------
// bl/dl rounded to 100 ms; d NOT rounded (exact ms)
// ---------------------------------------------------------------------------

/**
 * dl rounded to nearest 100 ms.
 * SetBufferLength(2350) at rate 1.0 -> dl = 2350 -> (2350+50)/100*100 = 2400.
 * Named test to explicitly document the rounding contract for dl.
 */
...
/**
 * d is NOT rounded — exact ms value passed through.
 * SetFragmentDuration(2150) -> d=2150; must NOT become d=2200.
 * Locks the "d is plain ms, never rounded" invariant.
...
// ---------------------------------------------------------------------------
// alpha-sort within each group with the full key set
// ---------------------------------------------------------------------------

/**
 * Object group alpha order with d present: br < d < ot < tb.
...
/**
 * Request group full alpha order with all keys present:
...
/**
 * Status group alpha order with both keys present: bs < rtp.
...
// ---------------------------------------------------------------------------
// su bare token present when true, absent when false
// ---------------------------------------------------------------------------

/**
 * su emitted as a bare token (no '=') when SetStartupUrgent(true).
 * Instance B (default false) must omit su entirely from CMCD-Request.
 * Pairs with the existing bs tests to give full bool-token coverage.
 */
...
// ---------------------------------------------------------------------------
// each key maps to its correct CTA-5004 group (new keys)
// ---------------------------------------------------------------------------

/**
 * d appears only in CMCD-Object; not in Request or Status.
...
/**
 * dl, mtp, and su appear only in CMCD-Request; not in Object or Status.
...
/**
 * rtp appears only in CMCD-Status; not in Object, Request, or Session.
...
// ---------------------------------------------------------------------------
// Regression: SetFragmentDuration(0) clears d — init-segment guard
// ---------------------------------------------------------------------------
 ...
 *
 * This mirrors the init-segment request path where the request calls
...
// ---------------------------------------------------------------------------
// AudioCMCDHeaders coverage — rtp, dl, mtp, su
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// dl with a negative playback rate (reverse trick play)
// ---------------------------------------------------------------------------
