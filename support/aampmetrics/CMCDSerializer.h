 ...
 *
 * Callers fill a std::vector<CMCDEntry> and pass it to SerializeToCMCDMap.
 ...
 * comma-delimited value string sorted alphabetically by key (CTA-5004 §3)
 * and stores it as the first (and only) element of the vector at the group's
 * header key in out.
 *
 * If a group key already exists in out, the new tokens are merged with the
 * pre-existing tokens, and the combined set is re-sorted alphabetically by
